...
def init_db():
    ...
    try:
        cursor.execute(
            "ALTER TABLE funnel ADD COLUMN node_id TEXT DEFAULT 'kiosk_default'"
        )
    except sqlite3.OperationalError:
        pass
...
async def process_analytics():
    ...
    init_db()
    logger.info(
        "Analytics Microservice listening for funnel events across all nodes..."
    )
            ...
            if message["type"] == "pmessage":
                ...
                if base_channel == "CUSTOMER_DETECTED":
                    ...
                    await asyncio.to_thread(
                        update_funnel, session_id, node_id, "APPROACHED", "", strategy
                    )
                    logger.info(
                        f"[Analytics] Session {session_id} on {node_id} entered funnel: APPROACHED using {strategy}"
                    )

                elif base_channel == "CUSTOMER_REPLY":
                    text = data.get("text", "")
                    await asyncio.to_thread(
                        update_funnel, session_id, node_id, "ENGAGED", text
                    )
                    logger.info(
                        f"[Analytics] Session {session_id} on {node_id} advanced funnel: ENGAGED"
                    )

                elif base_channel == "CUSTOMER_OBJECTION":
                    text = data.get("text", "")
                    await asyncio.to_thread(
                        update_funnel, session_id, node_id, "OBJECTION_RAISED", text
                    )
                    logger.info(
                        f"[Analytics] Session {session_id} on {node_id} funnel state: OBJECTION_RAISED"
                    )

                elif base_channel == "CUSTOMER_CONVERTED":
                    await asyncio.to_thread(
                        update_funnel, session_id, node_id, "CONVERTED"
                    )
                    logger.info(
                        f"[Analytics] Session {session_id} on {node_id} advanced funnel: CONVERTED"
                    )
